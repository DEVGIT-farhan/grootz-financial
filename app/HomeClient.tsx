...
const CONTACT_EMAIL = "tradewithgrootz@gmail.com";
...
export default function Home() {
  ...
  return (
    <main id="top" className="site-shell min-h-screen overflow-x-hidden">
      ...
      <section id="services" className="section"><div className="mx-auto w-[min(1180px,calc(100%-40px))]"><div className="section-heading"><div><p className="eyebrow"><span /> Services</p><h2>One Stop Solution<br /><i>for all your Financial needs.</i></h2></div><p>Each conversation begins with the life behind the decision: your goals, responsibilities and available time.</p></div><div className="service-grid mt-12">{services.map(([number, title, copy]) => <article key={title} className="service-card"><span>{number}</span><h3>{title}</h3><p>{copy}</p><a href="#contact">Discuss this <b>↗</b></a></article>)}</div></div></section>
